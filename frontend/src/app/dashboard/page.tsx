...
export default function DashboardWrapperPage() {
    ...
    fetch("http://localhost:8000/api/v1/jobs?page_size=1")
      ...
      .then((data) => {
        if (data.items && data.items.length > 0) {
          ...
          router.replace(`/analysis/${id}?tab=dashboard`);
        } else {
          setError(true);
        }
      })
  ...
  return (
    <PageFrame>
      <PlatformHeader eyebrow="System routing" title="Locating investment dashboard" />
      <div className="flex h-[calc(100vh-100px)] items-center justify-center">
        {error ? (
          ...
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-sm text-muted-foreground">Redirecting to latest dashboard...</p>
          </div>
        )}
      </div>
    </PageFrame>
